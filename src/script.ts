const placeHolder = document.querySelector("div.form-div > form > div.enter-email > div") as HTMLDivElement;
const input = document.querySelector("div.form-div > form > div.enter-email > input") as HTMLInputElement;
const form = document.getElementsByTagName("form")[0];
const warningParagraph = document.querySelector("p.warning") as HTMLParagraphElement;
const loader = document.querySelector("div.loader") as HTMLDivElement;
const cover = document.querySelector("div.cover") as HTMLDivElement;
let a: number = 0;
let b: number = 0;
...
form.addEventListener("submit", (event) => {
    if (input.value === "" || input.value.length < 3) {
        ...
        (async () => {
            ...
            if (b === 0) {
                const h1 = document.querySelector("section > h1") as HTMLHeadingElement;
                const forgotEmailOrShowPass = document.querySelector("p.forgot-email") as HTMLParagraphElement;
                const createAccountOrForgotPass = document.querySelector("div.submit-form > p") as HTMLParagraphElement;
                setTimeout(() => {
                    ...
                    `;
                    const checkbox = forgotEmailOrShowPass.getElementsByTagName("input")[0] as HTMLInputElement;
